...
class Review:
    def __init__(self, id, steamid, date, hours, last_two_weeks, last_played, content, comments, helpful, funny, recommended
                 , game_name):
        self.id = id
    ...
    def get_content(self):
        return self.id, self.steamid, self.date, self.hours, self.last_two_weeks, self.last_played, self.content, \
...
def get_reviews(appID, game_name):
    ...
    while True:
        try:
            ...
            if response['query_summary']['num_reviews'] == 0 or len(reviews_list) > 10000:
                break
            else:
                for item in response['reviews']:
                    if int(item['author']['playtime_last_two_weeks']) > 60:
...
def json_string(id, steamid, date, hours, last_tow_weeks, last_played, content, comments, helpful, funny, recommended, gameName):
...
def get_reviews_run(appID, game_name):
    ...
    for item in review_ids:
        ...
        id, steamid, date, hours, last_tow_weeks, last_played, content, comments, helpful, funny, recommended, \
        gameName = review_ids[review_ids.index(item)].get_content()

        _ = json_string(id, steamid, date, hours, last_tow_weeks, last_played, content, comments, helpful, funny,
